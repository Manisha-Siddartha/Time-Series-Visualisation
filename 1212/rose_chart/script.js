...
Chart.rose = function() {
	...
	var margin = {'top': 1, 'right': 2, 'bottom': 40, 'left': 2},
		height = 500,
		width = 500,
		color = 'rgb(0,0,0)',
	...
	function createWedges( data ) {
		...
		wedges = wedgeGroups.selectAll('.wedge')
		  ...
		  .enter().append('svg:path')
		  	.attr('class', function(d) { return 'wedge ' + d.legend +' '+d.label; })
		  	.attr('d', arc );
		...
		wedges.on('mouseover',mousemoved)
		...
		wedgeGroups.selectAll('.label')
		  ...
		  .enter().append('svg:text')
	  		...
	  		.append('textPath')
	  			...
	  			.on("click", function(d,i) { barchart(d.label);});
